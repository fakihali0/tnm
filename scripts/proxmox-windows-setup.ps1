...
if ($existingRuleOrch) {
    Write-Host "[✓] Orchestrator firewall rule already exists" -ForegroundColor Green
} else {
    try {
        New-NetFirewallRule `
            -DisplayName $FirewallRuleOrchestrator `
            -Direction Inbound `
            -LocalPort $OrchestratorPort `
            -Protocol TCP `
            -Action Allow `
            -Description "Allow inbound traffic on port $OrchestratorPort for MT5 Container Orchestrator" | Out-Null
        Write-Host "[✓] Orchestrator firewall rule created for port $OrchestratorPort" -ForegroundColor Green
    } catch {
...
if ($existingRuleMT5) {
    Write-Host "[✓] MT5 containers firewall rule already exists" -ForegroundColor Green
} else {
    try {
        New-NetFirewallRule `
            -DisplayName $FirewallRuleMT5Range `
            -Direction Inbound `
            -LocalPort "$MT5PortRangeStart-$MT5PortRangeEnd" `
            -Protocol TCP `
            -Action Allow `
            -Description "Allow inbound traffic on ports $MT5PortRangeStart-$MT5PortRangeEnd for dynamic MT5 containers" | Out-Null
        Write-Host "[✓] MT5 containers firewall rule created for ports $MT5PortRangeStart-$MT5PortRangeEnd" -ForegroundColor Green
    } catch {
...
# Dockerfile for MT5 Container Orchestrator
# escape=``
...
try {
    ...
    Write-Host "[✓] Orchestrator Dockerfile created" -ForegroundColor Green
} catch {
...
$mt5Dockerfile = @"
# Dockerfile for MT5 Data Service (Template for dynamic containers)
# escape=``
...
# Run MT5 data service
CMD ["python", "-m", "uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8000"]
"@

try {
    $mt5Dockerfile | Out-File -FilePath "$mt5ServiceDir\Dockerfile" -Encoding UTF8 -Force
    Write-Host "[✓] MT5 Service Template Dockerfile created" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "[✓] Orchestrator docker-compose.yml created" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "[✓] Orchestrator requirements.txt created" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "[✓] MT5 Service requirements.txt created" -ForegroundColor Green
} catch {
