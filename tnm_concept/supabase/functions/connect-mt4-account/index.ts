...
interface MT4ConnectionRequest {
  server: string;
  login: string;
  password: string;
  broker_name: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { server, login, password, broker_name }: MT4ConnectionRequest = await req.json();

    // SECURITY: Use secure logging that masks sensitive data (NEVER log passwords)
    secureLog('Attempting MT4 connection:', { server, login, broker_name });
    ...
    try {
      await supabaseClient.rpc('log_security_event', {
        _event_type: 'mt4_connection_attempt',
        _details: {
          user_id: user.id,
          broker: broker_name,
          server: server,
    ...
    // Use MetaAPI or similar service for real MT4 connection
    ...
    if (!createAccountResponse.ok) {
      ...
      console.error('MetaAPI account creation failed:', errorData);
    ...
    if (!deployResponse.ok) {
      ...
      // Log full error server-side for debugging
      console.error('MetaAPI deployment failed:', errorData);
      // Throw generic error that will be sanitized
      throw new Error(errorData.message || 'Deployment failed');
    }

    // Step 3: Wait for connection and get account info
    let accountInfo: MT4AccountInfo | null = null;
    let retries = 0;
    ...
    while (!accountInfo && retries < maxRetries) {
      try {
        ...
        console.log(`Retry ${retries + 1}: Waiting for account connection...`);
      }
    ...
    if (!accountInfo) {
      throw new Error('Failed to retrieve account information after connection');
    }
    ...
    secureLog('Successfully connected MT4 account:', { account_id: dbAccount.id });

    // Log successful connection for security monitoring
    try {
      await supabaseClient.rpc('log_security_event', {
        _event_type: 'mt4_connection_success',
        _details: {
          user_id: user.id,
          account_id: dbAccount.id,
    ...
    console.error('Error connecting MT4 account:', error);
    ...
    try {
      ...
      await supabaseClient.rpc('log_security_event', {
        _event_type: 'mt4_connection_failed',
        _details: {
